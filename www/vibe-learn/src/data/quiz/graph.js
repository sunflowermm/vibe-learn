...
function buildQuizGraphModelUncached() {
  ...
  packs.forEach((pack, di) => {
    ...
    frames.push({
      ...
      kind: 'chapter',
      label: dom.label,
      subtitle: `${children.filter((c) => c.role === 'set' && c.kindFilter === 'concept').length} 概念 · ${children.filter((c) => c.role === 'set' && c.kindFilter === 'interview').length} 大厂 · 池 ${pool.questionCount}`,
      tag: '领域',
