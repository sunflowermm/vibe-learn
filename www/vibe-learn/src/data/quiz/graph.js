...
function buildQuizGraphModelUncached() {
  ...
  const packs = domains.map((dom) => {
    ...
    const children = [
      ...
      ...domainSets.map((s) => ({
        id: s.id,
        label: s.title,
        subtitle: s.caption || (s.kind === 'interview' ? '大厂开口' : '概念细节'),
        tag: `${s.kind === 'interview' ? '大厂' : '概念'} · ${s.questions.length}`,
        role: 'set',
        setKind: 'set',
  ...
  packs.forEach((pack, di) => {
    ...
    frames.push({
      ...
      kind: 'chapter',
      label: dom.label,
      subtitle: `本章共 ${pool.questionCount} 题 · ${children.filter((c) => c.role === 'set' && c.kindFilter === 'concept').length} 概念组 · ${children.filter((c) => c.role === 'set' && c.kindFilter === 'interview').length} 大厂`,
      tag: '领域',
