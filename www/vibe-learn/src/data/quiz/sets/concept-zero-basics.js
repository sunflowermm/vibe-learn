  ...
  questions: [
    ...
    },
    {
      ...
      choices: [
        ...
        { t: '文件夹不能包含文件', ok: false, why: '目录正是用来装文件与子目录的。' },
        { t: '所有文件必须叫 a.txt', ok: false, why: '名字可自定；扩展名只是约定提示用途。' },
        { t: '路径对程序不重要', ok: false, why: '路径错了就找不到文件，程序跑不起来。' },
      ],
    ...
    },
    {
      ...
      choices: [
        { t: '编辑→保存→在正确目录用正确运行时执行→看到输出的闭环', ok: true, why: '环境与路径信心比代码长短更重要；闭环通了才能往下排障。' },
        { t: '一次写出完整电商网站的能力', ok: false, why: '目标过大；第一程序只验证工具链。' },
        { t: '证明以后可以不装 Node / 编辑器也能交付', ok: false, why: '仍需编辑器与运行时；这次成功恰恰说明它们在位。' },
        { t: '复制能跑就永远不必再读任何一行', ok: false, why: '能跑是开始；理解才能自己改代码和排障。' },
      ],
