  ...
  questions: [
    ...
    },
    {
      ...
      choices: [
        ...
        {
          t: '直接改 src/infrastructure 里的框架源码，把业务逻辑写进 Runtime',
          ok: false,
          why: 'Core 开发者不得改 Runtime；缺能力扩基类或提框架改动。',
        },
        {
          t: '仓库根目录任意文件夹随便放，只要文件名像插件就会被 Loader 加载',
          ok: false,
          why: '随意放置不会被扫描。',
        },
        {
          t: '只写在聊天窗口，由 Agent 注入运行中进程，不必落盘到 core/ 子目录',
          ok: false,
    ...
    },
    {
      ...
      choices: [
        ...
        {
          t: '只改 data/<产品>/ 一份运行时 yaml 永远够用，新环境不必模板与 schema',
          ok: false,
          why: '新 clone 缺模板；字段无 schema；代码可能未 read。',
        },
        {
          t: '只改 README 说明，再在代码里硬编码同名字符串，即可替代配置系统',
          ok: false,
          why: '文档不驱动运行时。',
        },
        {
          t: '三同步指 Git 的 add/commit/push 三次操作，与配置模板无关',
          ok: false,
    ...
    },
    {
      ...
      choices: [
        ...
        {
          t: '所有独立产品业务配置都应塞进 config/default_config/，不要用 core/<名>/default/',
          ok: false,
          why: '独立产品模板在自己的 default/。',
        },
        {
          t: 'default_config 只放前端 CSS 主题，服务端端口与密钥全部硬编码进源码',
          ok: false,
          why: '该目录是服务端 YAML 模板。',
        },
        {
    ...
    },
    {
      ...
      choices: [
        ...
        {
          t: '仓库根目录建 shared/ 当应用名最短最好，挂载路径也更短更习惯',
          ok: false,
          why: '保留名；产品用自有应用名。',
        },
        {
          t: '静态页只能放 src/factory，core/*/www 下的页面不会被挂载访问',
          ok: false,
          why: 'www 属 Core 业务前端。',
        },
        {
          t: 'Core 禁止 HTML 静态页，必须全部外链 CDN，仓内不能有 www 目录',
          ok: false,
    ...
    },
    {
      ...
      choices: [
        ...
        {
          t: '只背某一个源码文件的绝对路径即可交差，不必理解 Runtime、Core 与 plugin/http/workflow 等扩展点如何分工',
          ok: false,
          why: '地图是职责边界，不是死记文件名。',
        },
        {
          t: '证明所有产品能力都应写进同一个 plugin 文件里，不必再拆 http、workflow、www 或 commonconfig 目录',
          ok: false,
          why: '扩展点拆分正是为了职责清晰。',
        },
        {
          t: '业务层全景只讲前端配色、字体与文案语气，不涉及服务端扩展点、Loader 扫描与配置归属纪律',
          ok: false,
          why: '讲的是服务端扩展点与放码。',
        },
      ],
    ...
    },
    {
      ...
      choices: [
        ...
        {
          t: '任意模块被 import 的瞬间即可假定配置齐全，不必等 CommonConfigRegistry.load()',
          ok: false,
          why: '加载顺序决定就绪时机。',
        },
        {
          t: '只能在浏览器 www 里读 runtimeConfig，服务端插件与 http 禁止读取',
          ok: false,
          why: '它是服务端单例。',
        },
        {
          t: '永远禁止读 runtimeConfig，端口与密钥只能硬编码进业务模块',
          ok: false,
