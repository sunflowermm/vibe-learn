  ...
  questions: [
      ...
      choices: [
        {
          ...
          ok: true,
          why: '复现→二分/分层→验证，比随机改代码或清日志盲重启高效。',
        },
        {
          t: '遇到报错第一步就清空全部应用日志并盲目重启服务，不必复现',
          ok: false,
          why: '清日志/盲重启会丢掉证据；应先复现并定环境层还是业务层。',
        },
        {
    ...
    },
    {
      ...
      choices: [
        ...
        {
          t: '单元测试覆盖率、静态类型检查、Code Review 通过率，按场景组合使用',
          ok: false,
          why: '那是质量门禁常见项，不是可观测性三件套。',
        },
        {
          t: '负载均衡、自动扩缩容、蓝绿发布策略，按场景组合使用',
          ok: false,
          why: '那是流量与发布手段，不是 logs/metrics/traces。',
        },
        {
          t: '需求文档、架构图、接口契约说明书，按场景组合使用',
          ok: false,
          why: '那是设计资产；可观测性看运行时信号。',
        },
      ],
    ...
    },
    {
      ...
      choices: [
        ...
        {
          t: '只留 level=error 一个词、去掉时间戳与请求 id，避免日志过长',
          ok: false,
          why: '无法把多机事件排成时间线。',
        },
        {
          t: '写入完整卡号与证件号作「上下文」，方便客服人工核对订单',
          ok: false,
          why: '敏感字段应脱敏；泄漏本身是事故。',
        },
        {
          t: '只打本机墙钟、不写 request/trace id，靠人工猜哪条请求',
          ok: false,
          why: '跨服务关联依赖稳定的请求/追踪标识。',
        },
      ],
