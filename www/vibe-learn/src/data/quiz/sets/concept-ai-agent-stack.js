  ...
  questions: [
    {
      ...
      choices: [
        ...
        {
          t: '可执行二进制补丁；模型直接改操作系统内核',
          ok: false,
          why: '模型无直接系统权限，必须经过宿主。',
        },
        {
          t: '只能输出 JSON，此后禁止任何自然语言回复',
          ok: false,
          why: '工具调用与自然语言可交替出现。',
        },
        {
    ...
    },
    {
      ...
      choices: [
        ...
        {
          t: '只能做静态补全，禁止调用工具或多轮迭代',
          ok: false,
          why: 'Agent 价值正在于多步工具与状态循环。',
        },
        {
          t: '控制循环只发生在训练期，推理期不再出现',
          ok: false,
          why: '控制循环是运行/推理期行为。',
        },
        {
          t: '与 RAG 检索互斥，用了 Agent 就不能检索',
          ok: false,
    ...
    },
    {
      ...
      choices: [
        ...
        {
          t: '禁止工具调用，只把向量库连接串写进 system 提示',
          ok: false,
          why: '密钥进提示既不安全，也不是工具环。',
        },
        {
          t: '必须先关闭 maxToolRounds，只允许单轮 tool_calls',
          ok: false,
          why: '多轮再检索正需要工具轮次预算。',
        },
        {
          t: '固定「检索→拼 prompt→生成」三件套，模型不得再决定读哪段',
          ok: false,
    ...
    },
    {
      ...
      choices: [
        ...
        {
          t: '存放生产数据库密码，方便 Agent 自动连库',
          ok: false,
          why: '密钥禁止进仓；AGENTS.md 只写规则与边界。',
        },
        {
          t: '替代 pnpm-lock.yaml 锁定依赖版本',
          ok: false,
          why: '依赖锁定靠 lockfile；AGENTS.md 是行为说明。',
        },
        {
          t: '仅供离线训练语料，IDE 推理时不会读取',
          ok: false,
    ...
    },
    {
      ...
      choices: [
        ...
        {
          t: 'L1 必须先实现完整 A2A 协作，否则不能调模型',
          ok: false,
          why: 'L3 协作不是对话入口前提。',
        },
        {
          t: 'L2 等于取消工具调用，只允许纯文本补全',
          ok: false,
          why: 'L2 正是工具面，不是禁用工具。',
        },
        {
          t: 'L1 与 L2 都指 DNS 解析流程',
          ok: false,
    ...
    },
    {
      ...
      choices: [
        ...
        {
          t: '替代 Rules，禁止在项目里写 .cursor/rules',
          ok: false,
          why: 'Rules 约束行为；registerMCPTool 暴露可调用能力，职责不同。',
        },
        {
          t: '只允许本地 stdio MCP，禁止 remote-mcp 走网络',
          ok: false,
