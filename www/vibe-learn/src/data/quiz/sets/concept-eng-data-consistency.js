  ...
  questions: [
    {
      ...
      choices: [
        ...
        {
          t: '缓存永远比数据库正确，应以缓存为准永久对外服务',
          ok: false,
          why: '缓存常过期或脏读，不能当最终真相。',
        },
        {
          t: '不一致可以永久忽略，业务层不必定义失效与重建策略',
          ok: false,
          why: '用户会看到错账/错状态，业务会出错。',
        },
        {
          t: '有了 Redis 就不需要数据库，权威持久与复杂查询也可省略',
          ok: false,
    ...
    },
    {
      ...
      choices: [
        ...
        {
          t: '数据库会自动回滚已经成功发出的邮件或外部 HTTP 副作用',
          ok: false,
          why: '库事务管不到已发出的外部副作用。',
        },
        {
          t: '外部副作用永远应塞进库事务里同步打外部 HTTP，才能保证一致',
          ok: false,
          why: '易拖垮事务与锁；外部慢/失败会放大故障。',
        },
        {
          t: '「本地已提交、外部副作用失败」这类情况在生产环境几乎不存在',
          ok: false,
          why: '提交后副作用失败在生产天天出现。',
        },
      ],
    ...
    },
    {
      ...
      choices: [
        ...
        {
          t: "只要建了索引就不可能出现 N+1，ORM 会自动合并所有查询",
          ok: false,
          why: '这是工程性能术语，不是数学证明题。',
        },
        {
          t: "N+1 是数学恒等式证明，与应用层循环查询模式无关",
          ok: false,
          why: '索引加快单次查询，消不掉「查了 N+1 次」。',
        },
        {
          t: "N+1 只会出现在 MongoDB，关系库用 JOIN 就永远不会有",
          ok: false,
          why: '关系库 + ORM 场景更经典。',
        },
      ],
    ...
    },
    {
      ...
      choices: [
        ...
        {
          t: "所有分布式系统必须强一致否则非法，最终一致在合规场景禁用",
          ok: false,
          why: '最终一致会收敛；「从不」是另一回事。',
        },
        {
          t: "最终一致等于数据永不收敛，只能靠人工对账永久擦除差异",
          ok: false,
          why: '要在一致性、可用与成本之间权衡。',
        },
        {
          t: "缓存 TTL 与一致性无关：过期策略不会影响用户是否读到旧值",
          ok: false,
