  ...
  questions: [
    {
      ...
      choices: [
        ...
        {
          t: 'events 监听器可以取代全部对外 http 路由与鉴权门面',
          ok: false,
          why: 'HTTP 仍是对外产品面。',
        },
        {
          t: 'events 与 Tasker 同义：都是协议适配层，可以互相替换使用',
          ok: false,
          why: 'Tasker 接通道协议；events 听进程内事件。',
        },
        {
          t: '把全部用户聊天指令写进 events Listener，用监听器替代 plugin 指令面',
          ok: false,
          why: '指令业务在 plugin/。',
        },
      ],
    ...
    },
    {
      ...
      choices: [
        ...
        {
          t: 'Tasker 应替代 plugin 堆领域业务逻辑，通道适配与指令写在同一层',
          ok: false,
          why: 'Tasker 只管通道适配。',
        },
        {
          t: 'Tasker 等于通用 REST 网关，专门对外暴露 HTTP JSON API',
          ok: false,
          why: '消息通道 ≠ HTTP 产品面。',
        },
        {
          t: '有了 Tasker 适配通道后就不必再做鉴权，协议层已足够安全',
          ok: false,
          why: '通道仍有信任边界与权限。',
        },
      ],
    ...
    },
    {
      ...
      choices: [
        ...
        {
          t: '本仓禁止使用 Python：子服也不能跑 Python 工具链或 apis',
          ok: false,
          why: 'pyserver / apis 正是 Python 子服路径。',
        },
        {
          t: '子服运维命令必须从主服 stdin 转发，禁止在子服终端直接输入',
          ok: false,
          why: '子服终端「子服>」直接输入。',
        },
        {
          t: '主服编排进程跑在浏览器里，没有独立的 Node AgentRuntime',
          ok: false,
          why: '主服是 Node 进程。',
        },
      ],
    ...
    },
    {
      ...
      choices: [
        ...
        {
          t: '有了 SQLite 本地持久就不必再装 Redis，热缓存与会话也可省略',
          ok: false,
          why: '热数据路径依赖 Redis，不是「二选一」。',
        },
        {
          t: 'Redis 与 SQLite 都必须改成 MongoDB，否则 Runtime 拒绝启动',
          ok: false,
          why: 'Mongo 是可选 Core，不是 Runtime 硬前置。',
        },
        {
          t: '本仓数据面只能写在浏览器 localStorage，服务端禁止 Redis/SQLite',
          ok: false,
    ...
    },
    {
      ...
      choices: [
        ...
        {
          t: '每个插件自己再装一份 sqlite3 npm 包，绕过 Runtime 全局封装自行管理连接与生命周期',
          ok: false,
          why: '禁止另起炉灶替代约定实现。',
        },
        {
          t: '把 Redis/SQLite 连接串与密码写进 www 前端打包产物，方便浏览器页面直连数据库',
          ok: false,
          why: '浏览器可见即泄漏。',
        },
        {
          t: '跨 Redis 与 SQLite 默认提供统一分布式事务，业务代码不必自己处理一致性边界',
          ok: false,
          why: '跨引擎仅最终一致，无统一事务。',
        },
      ],
