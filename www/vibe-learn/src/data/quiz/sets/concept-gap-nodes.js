  ...
  questions: [
    {
      ...
      choices: [
        ...
        {
          t: '规则和技能是同一类文档，放进 rules 或 skills 目录效果完全一样',
          ok: false,
    ...
    },
    {
      ...
      choices: [
        {
          t: "二分查找等算法要求数据先有序（或可比较序）；乱序上硬套会得到错误结论",
          ok: true,
          why: '模板有边界条件：是否稳定排序、是否允许重复、查找区间开闭等，都要先说清。',
        },
        {
          t: '只要数组长度大于 2，任何查找都可以宣称是二分且保证正确',
          ok: false,
          why: '二分依赖有序与正确的中点收缩；长度不是充分条件。',
        },
        {
          t: '排序永远比查找重要，所以面试从不需要讨论查找模板',
          ok: false,
          why: '二者常成对出现：先排序再查找，或在有序结构上直接查。',
        },
        {
          t: '快排保证最坏情况也是线性时间，因此可以不谈复杂度边界',
          ok: false,
    ...
    },
    {
      ...
      choices: [
        {
          t: "先定义状态，再写转移（或证明贪心选择性质），最后谈边界与复杂度",
          ok: true,
          why: 'DP 靠状态转移；贪心要证明局部最优能推到全局。开口按这三步不易跑偏。',
        },
        {
          t: '先把标准答案背下来，状态定义和转移方程可以面试临场再想',
          ok: false,
          why: '背题遇变形就崩；状态与转移才是可迁移的能力。',
        },
        {
          t: '贪心一定优于动态规划，所以永远应优先写贪心解法',
          ok: false,
          why: '许多题贪心不正确；需要证明或退回 DP/搜索。',
        },
        {
          t: '动态规划只能用于游戏得分计算，与工程调度类问题无关',
          ok: false,
