...
export default defineQuizSet({
  ...
  questions: [
    ...
    },
    {
      ...
      choices: [
        { t: 'const 优先；需要再赋值再用 let；少用 var', ok: true, why: '缩小意外重绑与函数作用域坑。' },
        { t: '只用 var，禁止 const', ok: false, why: '社区已转向 let/const。' },
        { t: '变量必须全部挂到 globalThis', ok: false, why: '污染全局。' },
        { t: '声明关键字可以随便造', ok: false, why: '语法固定。' },
      ],
    ...
    },
    {
      ...
      choices: [
        { t: '引用共享与浅拷贝：改「副本」可能改到原对象', ok: true, why: '会话状态、缓存、工具参数常被偷偷改写。' },
        { t: '数组下标必须从 1 开始否则 JSON 非法', ok: false, why: 'JS 从 0；JSON 数组无下标字段。' },
        { t: '对象不能嵌套', ok: false, why: '可以嵌套。' },
        { t: 'map/filter 一定深拷贝整棵树', ok: false, why: '元素仍可能是同一引用。' },
      ],
    ...
    },
    {
      ...
      choices: [
        { t: '边界清晰、可复用、依赖显式，避免单文件巨石', ok: true, why: '与「函数封装」同一思想，升到文件级。' },
        { t: '模块化后永不需要测试', ok: false, why: '仍要测。' },
        { t: 'import 会自动部署到生产', ok: false, why: '否。' },
        { t: '有模块就不需要 Git', ok: false, why: '无关。' },
      ],
    ...
    },
    {
      ...
      choices: [
        { t: 'parse：字符串→对象；stringify：对象→字符串——API 与本地存储常成对出现', ok: true, why: '非法 JSON 会抛错，要 try/catch 或校验。' },
        { t: 'parse 用于加密', ok: false, why: '否。' },
        { t: 'stringify 会执行字符串里的代码', ok: false, why: '那是 eval 的危险，不是 stringify。' },
        { t: '二者只能用于 YAML', ok: false, why: '面向 JSON。' },
      ],
    ...
    },
    {
      ...
      choices: [
        { t: '用同步外形写异步流程，错误可用 try/catch 收敛', ok: true, why: '底层仍是 Promise；要懂并发与失败路径。' },
        { t: 'await 会让 CPU 真的休眠操作系统', ok: false, why: '等待的是 Promise 完成，事件循环继续。' },
        { t: '有了 await 就不需要处理拒绝（reject）', ok: false, why: '仍要 catch。' },
        { t: 'async 函数不能返回 Promise', ok: false, why: '它总是返回 Promise。' },
      ],
    ...
    },
    {
      ...
      choices: [
        { t: '某个对象是 undefined/null，却还在取属性——对一下数据与可选链', ok: true, why: '读报错类型+行号，再查数据来源。' },
        { t: '一定是路由器硬件坏了', ok: false, why: '应用层数据问题。' },
        { t: '表示 Git 冲突标记未删', ok: false, why: '那是另一类报错。' },
        { t: '可以忽略，浏览器会自动补对象', ok: false, why: '不会。' },
      ],
