  ...
  questions: [
    ...
    },
    {
      ...
      choices: [
        ...
        {
          t: '先删掉整个 node_modules 并换一台电脑重装，再考虑看报错',
          ok: false,
          why: '依赖重装有时有用，但第一步仍应读类型与行号，避免盲目重装。',
        },
        {
          t: '先把仓库强制 reset 到很久以前的提交，丢掉当前改动再猜',
          ok: false,
          why: '未定位前硬回滚可能丢掉线索与未提交工作。',
        },
        {
          t: '忽略所有报错信息，反复点运行直到偶然成功就算修好',
          ok: false,
    ...
    },
    {
      ...
      choices: [
        ...
        {
          t: '一定是磁盘空间写满，操作系统开始随机返回 undefined',
          ok: false,
          why: '磁盘满通常是 ENOSPC 类 I/O 错，不是「对 undefined 取属性」。',
        },
        {
          t: '说明 === 写错成了 ==，比较运算符会直接抛这种 TypeError',
          ok: false,
          why: '==/=== 一般返回布尔值，不会单独以「读 undefined 属性」表现。',
        },
        {
          t: '只要把文件另存为 .ts 扩展名，运行时就会自动补上缺失对象',
          ok: false,
          why: '扩展名不造数据；仍要保证运行时对象存在或可选链/默认值。',
        },
      ],
