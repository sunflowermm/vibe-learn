...
导图2 **后端区**里大量「站点上线」名词，与本框协议故事对照：

| 本框节点 | 导图2 对照 | 记住 |
|----------|------------|------|
| HTTP / Web | **HTTP** · **HTTPS** · **URL** · 重定向 | 应用层说话 ≠ 已加密 |
| DNS / HTTPS | **DNS** · **域名** · **HTTPS** | 名字解析与证书是两层 |
| 反代 / CDN | **CDN** · 重定向 · 部署 | CDN 是副本靠近用户，不是「另一种 HTTP」 |
| 端口 / 安全组 | **端口** | 要写清 **协议+端口**（含 UDP 开服） |
| API 与前后端 | **路由** · **CORS** · **JSON** | 前后端分界常落在 HTTP API |

跨导图点开词条补直觉；**安全组、反代、证书验收仍以本框实务课为准**。
