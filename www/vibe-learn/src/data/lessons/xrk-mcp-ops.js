...
\`\`\`quiz
{"title":"MCP 运维","questions":[{"q":"本仓 MCP 工具主要应在哪一类文件里注册？","choices":[{"t":"core/*/workflow 的 registerMCPTool","ok":true,"why":"汇总进 mcp-server。"},{"t":"写在 www 静态 JS 里，由浏览器在页面加载时注册工具","ok":false,"why":"前端不是工具注册面。"},{"t":"必须改 Linux 内核模块才能挂工具","ok":false,"why":"应用层协议。"},{"t":"只能写在子服且主服禁止执行任何工具","ok":false,"why":"现行以主服工具执行为准。"}]},{"q":"外部 Client 连不上时，第一刀？","choices":[{"t":"先看主服是否起来、MCP 挂载日志、Key 与 URL 路径","ok":true,"why":"运维核对表。"},{"t":"先微调基座权重，期望连口问题随之消失","ok":false,"why":"连口都没有。"},{"t":"删掉全部 plugin 再重启，当作标准排障步骤","ok":false,"why":"不对症。"},{"t":"把 Key 贴进公开 Issue 或群聊，方便别人代查","ok":false,"why":"泄漏。"}]},{"q":"策略、扫描与审批门禁主要卡在运行时哪一层？","choices":[{"t":"只在 Cursor 插件客户端里拦截，服务端不管","ok":false,"why":"运行时门禁。"},{"t":"MCPServer.handleToolCall（各入口共用）","ok":true,"why":"LLM/HTTP/WS 一条路。"},{"t":"只在 Nginx 配置里用 rewrite 做审批","ok":false,"why":"无关。"},{"t":"只在 Vite 构建时静态扫描，运行时不再门禁","ok":false,"why":"无关。"}]}]}
\`\`\`
