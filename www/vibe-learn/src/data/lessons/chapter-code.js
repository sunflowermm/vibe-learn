...
本框练的是**会写、会跑、会读报错**；导图2 的 **JavaScript / 终端 / 环境变量** 词条适合当「名词注解」，不要替代过关：

| 本框目标 | 导图2 可对照 | 本框仍要自己做 |
|----------|--------------|----------------|
| \`node\` 跑通第一文件 | JavaScript · 终端 | 自己保存文件并执行 |
| JSON / env | JSON · 环境变量 | 手改一份并知道勿提交密钥 |
| 异步与模块 | JavaScript（进阶说法） | 用过关题验证，不靠「听懂」 |

Agent 当教练时：约束写清「先提示再评审、勿完整答案」；词表细节可让它按导图2 口吻解释，但**验收以本机输出为准**。
