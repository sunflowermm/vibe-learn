...
> **学会之后**：能复述 Runtime Redis+SQLite 必需与可选 Core 边界。
...
> 导图2 常把「数据库」喊进技术栈；本课钉 **本仓热路径契约**。产品直觉回番外数据库。

| 导图2 | Vibe 口语 | 本仓专业落点 |
|-------|-----------|--------------|
| **后端** | 服务端状态 | Runtime **Redis + SQLite** 必需；业务库走可选 Core |
| **部署上线** | 环境齐套 | Redis 装机见部署环境；概念见番外数据库 |
| **技术栈** | 数据层选型 | 勿把「会装 MySQL」说成「本仓必需」 |
