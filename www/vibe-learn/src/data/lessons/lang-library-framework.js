...
- 框架轨任选 Vue / Spring / …
## 导图2 · 组件 / 前端 / 后端 × 库·框架·运行时·中间件

> 导图2 的组件/前后端帮你对齐口语；本课钉 **四层分类**（库/框架/运行时/中间件）。

| 导图2 | Vibe 口语 | 本课专业落点 |
|-------|-----------|--------------|
| **组件** | 可复用块 | UI 组件≈框架层产物；本仓 Core 扩展点是另一类「可插拔」 |
| **前端 / 后端** | UI 侧 / 服务侧 | 两侧都可能有库与框架；宿主语言仍要说清 |
| **技术栈** | 组合 | 中间件（Redis 等）常进栈，但不是语言/框架 |
