...
> **学会之后**：能读方法/状态码/Header 直觉，并优先用 HTTP 语义排 401/404。
...
| 能力 | 成功信号 |
|------|----------|
| 请求响应 | 能拆方法、路径、Header、Body |
| 状态码 | 2xx/3xx/4xx/5xx 分层；401/404/502 先查哪一层 |
| 鉴权与跨域 | 401 先查凭证；CORS 与 Cookie 边界各一句 |
| 本仓 | API 多在 HTTP；www 静态挂载另见第四章 |
| 跟 Agent | 可贴 curl 草稿；状态码与鉴权须你核对 |
...
## 导图2 · HTTP / HTTPS / URL / 重定向 × Web 语义

> 导图2 把站点上线口语钉得很全；本课钉 **方法·状态码·头·鉴权·CORS**。  
> **排障顺序以本课为准**（401≠换模型；502 先查上游）。

| 导图2 | Vibe 口语 | 本课专业落点 |
|-------|-----------|--------------|
| **HTTP** | 应用层说话 | 方法幂等直觉、状态码五类、Header/缓存 |
| **HTTPS** | 加密传输 | TLS 之上仍是 HTTP 语义；证书细节见 DNS/HTTPS 课 |
| **URL** | 定位资源 | 协议+主机+路径+查询；与路由/反代路径对齐 |
| **重定向** | 3xx 换地方 | 与缓存、登录跳转、HTTP→HTTPS 强相关 |
