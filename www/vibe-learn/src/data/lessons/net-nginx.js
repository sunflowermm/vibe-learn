...
> 官方入门：[nginx.org Beginner’s Guide](https://nginx.org/en/docs/beginners_guide.html)（\`proxy_pass\`）；负载均衡：[Using nginx as HTTP load balancer](https://nginx.org/en/docs/http/load_balancing.html)（\`upstream\`）。  
...
| 三件事 | 静态、反代、TLS 终止各一句 |
| 配置 | 认得 \`server\` / \`location\` / \`proxy_pass\` / \`upstream\` |
| 排障 | \`curl -I\` 分清 200 与 502；502 先查上游端口 |
| 本仓 | 开发可直连 Node；上线常 Nginx/面板反代 |
...
\`\`\`algo
{"kind":"revproxy","title":"Nginx 门面请求流","autoplay":true,"speed":850,"data":{"mode":"ok","client":"浏览器","proxy":"Nginx :443","upstream":"127.0.0.1:8080"}}
\`\`\`

\`\`\`mermaid
flowchart LR
  U[用户] -->|443 HTTPS| N[Nginx]
  N -->|静态 root/alias| FS[dist / www]
  N -->|proxy_pass| APP[127.0.0.1:业务端口]
\`\`\`

---

## 2. \`proxy_pass\` 与 \`upstream\`（官方心智）

Beginner’s Guide 最小形态：在 \`location\` 里写 \`proxy_pass http://localhost:8080;\`——匹配到的请求转给上游。

| 概念 | 直觉 |
|------|------|
| **\`proxy_pass\`** | 「这一段 URL 交给谁」；可写 IP:端口或命名 upstream |
| **\`upstream\`** | 一组后端；默认轮询（round-robin）；可 \`ip_hash\` / \`weight\` |
| **\`proxy_set_header\`** | 把真实 \`Host\`、客户端 IP（\`X-Forwarded-For\`）告诉上游 |
| **尾斜杠** | \`proxy_pass\` 带 URI 时会改写路径前缀——配错会 404，要对照文档 |
...
（证书路径与 \`ssl_certificate\` 见主机运维 **TLS** 课；此处只钉分流心智。）
...
| 504 Gateway Timeout | \`proxy_read_timeout\`；上游是否卡住 |
| 证书报错 | DNS 是否指到本机；80/443 是否放行 |
| 静态 404 | \`root\`/\`alias\` 与 \`try_files\` 路径 |
| 能本机访问、外网不行 | 安全组 / 防火墙是否只开了对的协议+端口 |

\`\`\`algo
{"kind":"revproxy","title":"502：上游无人听","autoplay":false,"speed":850,"data":{"mode":"fail","client":"curl -I","proxy":"Nginx","upstream":"端口配错 / 进程挂了"}}
\`\`\`
