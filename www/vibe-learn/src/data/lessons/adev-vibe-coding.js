...
> **学会之后**：能按五拍委派任务，守安全底线，并分清路径 A（写代码）与路径 B（第五章）。

## 学会之后（验收）

| 能力 | 成功信号 |
|------|----------|
| 五拍 | 目标→现场→约束→验收→复盘能口头走一遍 |
| 责任 | Agent 加速；验收与禁区仍是你的 |
| 分叉 | 写本仓走 A；造 Agent 应用走第五章 |
| 交底 | 指向项目记忆文件，不靠聊天永久记忆 |

\`\`\`check
{"title":"Vibe 通关","items":[{"id":"five","text":"能口头走完目标→现场→约束→验收→审 diff","hint":"五拍"},{"id":"own","text":"知道 Accept 与高危命令责任在你","hint":"责任"},{"id":"ab","text":"能分清路径 A（写代码）与第五章路径 B","hint":"分叉"},{"id":"mem","text":"知道交底写进文件，不靠聊天永久记忆","hint":"交底"}]}
\`\`\`

\`\`\`algo
{"kind":"vibefive","title":"Vibe 五拍：目标 → 现场 → 约束 → 验收 → 审 diff","autoplay":true,"speed":780}
\`\`\`
...
\`\`\`quiz
{"title":"反模式 + 导图2 判断","questions":[{"q":"Vibe Coding 里最危险的一句是？","choices":[{"t":"先 plan，再小范围改，再审 diff","ok":false,"why":"推荐节奏。"},{"t":"整个项目重构一下，不用看","ok":true,"why":"无边界、不可审。"},{"t":"这是堆栈原文，请指出文件:行号","ok":false,"why":"好提问。"}]},{"q":"AI 做出的活动报名页已能在本机打开，团队准备明天收集真实姓名和手机号。下一步更合适的是？","choices":[{"t":"暂停堆功能，先检查数据流、权限和依赖，补关键测试并由人审查改动","ok":true,"why":"导图2：本机画面正常只证明原型能运行；接触真实个人信息前，当正式软件检查。"},{"t":"继续接受 AI 的全部修改，只要页面没有报错就直接上线","ok":false,"why":"没有报错不能证明数据处理与权限安全。"},{"t":"先让 AI 把颜色和动画做精致，视觉完成后再默认功能可靠","ok":false,"why":"视觉完成度不能代替数据与越权检查。"}]}]}
\`\`\`
...
4. 路径 B → **第五章**；词表全量 → 面板 **跨导图**

## 本仓怎么做

| 概念 | 落点 |
|------|------|
| 五拍 | 任何委派默认带目标/现场/约束/验收 |
| 禁区 | 勿改 \`src/infrastructure\`；仅 pnpm；Node≥26 |
| 密钥 | \`.env\` 不进仓；聊天慎贴生产密钥 |
| 交底 | 根 \`AGENTS.md\` + \`.cursor/rules\`（见项目记忆课） |
`;
