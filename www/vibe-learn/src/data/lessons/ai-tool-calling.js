...
| 本课（本仓） | 导图2 | 钉死 |
|--------------|-------|------|
| 模型输出结构化意图 | **工具调用** | 执行权在运行时，不在「模型直接改盘」 |
| MCP 挂载与门禁 | **MCP** | 插座标准 ≠ 某一个 IDE |
| finalize 收口 | Agent 循环相关词（下一课） | 工具轮用尽仍可再请求正文 |

跨导图读工具调用 / MCP 词条的边界与 Agent 提示；本仓 \`handleToolCall\` 门禁以真源文档为准。
