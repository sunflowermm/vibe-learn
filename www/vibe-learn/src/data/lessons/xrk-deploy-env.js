...
> 导图2 把本课钉在 **部署上线 · 环境变量 · CI/CD · 预发 · 回滚**——下文用这些词对齐本仓清单，而不是空喊「上线」。  
> **学会之后**：能按清单自检 Git/Node/pnpm/Redis/浏览器引擎，能用部署/环境变量/CI 名词说话，并分清代理与本机服务。
...
| 能力 | 成功信号 |
|------|----------|
| 清单 | Git · Node≥26 · pnpm · Redis · 日常浏览器 |
| PATH / 环境变量 | 命令能找到；\`HTTP_PROXY\` 与系统代理分得清 |
| 部署对照 | 说出本仓「部署」=齐套环境+能 \`node app\`，不是只买 VPS |
| CI/CD 直觉 | CI=机器检查；CD=交付节奏；二者都不缩短本课清单 |
| 代理 | 境外才挂；本机 Redis/Node 勿误走代理 |
| 验收 | node app 能起；Redis 可 ping（若启用） |
...
> 导图2 部署族口语；本课钉 **本仓开演清单**（Git/Node/pnpm/Redis…），不把「买主机」当成部署完成。

| 导图2 | Vibe / 业界口语 | 本仓专业落点 |
|-------|-----------------|--------------|
| **部署上线（Deployment）** | 把可运行物放到用户能访问的环境 | 先绿本课五样；再谈域名/反代/面板；主服仍是 Node+pnpm |
| **环境变量（Env Var）** | 进程配置与密钥的载体 | PATH、\`HTTP(S)_PROXY\`/\`NO_PROXY\`、API Key；\`.env\` 不进 Git |
| **持续集成（CI）** | 推送后机器自动检查 | 应用 **pnpm** + 锁文件；红叉先读日志（工程素养 CI 课） |
| **持续交付 / 部署（CD）** | 可重复地推到某环境 | 不因「有流水线」省略 Redis/证书/备份 |
| **预发布（Staging）** | 类生产验证场 | 配置与数据卷要可复制；别拿生产密钥练手 |
| **回滚（Rollback）** | 出问题退回上一好版本 | 代码靠 Git；数据靠备份演练（主机运维·备份课） |

\`\`\`flip
{"title":"部署词条翻卡","cards":[{"front":"部署=买云主机？","back":"主机只是舞台；本课清单才是开演条件。"},{"front":"环境变量=写进源码？","back":"否；进进程环境/密钥库，不进公开仓。"},{"front":"CI 绿了就能裸奔端口？","back":"否；安全组/反代/证书另论。"},{"front":"回滚只回代码？","back":"数据卷与配置常更痛；要演练。"}]}
\`\`\`

> 面板会自动附上导图2 部署族词条正文。口播/自学时：**用词条对齐说法，用本课表格做验收**。

## 下一步

- **番外 · Vibe Coding 心智** — 部署/反代提问五拍  
- **番外 · 代理引擎（Clash）** — \`HTTP_PROXY\` 与系统代理；卡 GitHub 时优先回这里  
- **番外 · 本机目录** — Users / \`/home\` / \`.xxx\`  
- **不同终端环境**（第一章）— 终端 / Shell / OS  
- **首次跑通** — 最短命令串  
- **数据库** — Redis / SQLite 契约  
