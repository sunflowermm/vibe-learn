...
> **国内部署高频卡点：** clone / 下 GitHub Releases 前先打通代理（§0）；终端要单独设 \`HTTP(S)_PROXY\`（番外 Clash）。  
> 下载地址写在本课 / 思维导图卡片里，便于**复制**；口播视频**不念 URL**，可提示「去思维导图复制」或本机浏览器搜索。  
> 更深的「终端是什么」见第一章 **不同终端环境**；本课补 **部署场景下** 要用到的基础与手顺。  
> **学会之后**：能按清单自检 Git/Node/pnpm/Redis/浏览器引擎，能分清代理与本机服务。
...
| 口语 | 本仓落点 |
|------|----------|
| **部署上线** | 先绿本课五样；再谈域名/反代/面板；主服仍是 Node+pnpm |
| **环境变量** | PATH、\`HTTP(S)_PROXY\`/\`NO_PROXY\`、API Key；\`.env\` 不进 Git |
| **CI** | 应用 **pnpm** + 锁文件；红叉先读日志 |
| **CD / 预发 / 回滚** | 不因流水线省略 Redis/证书/备份；数据卷要可演练 |

\`\`\`decide
{"title":"清单绿了还缺什么？","start":"s","caption":"五样齐 ≠ 对公网安全。","steps":[{"id":"s","q":"本机 node app 已能起，下一步？","options":[{"label":"直接把业务端口裸奔公网","next":"bad"},{"label":"先谈反代/TLS/只开 80/443","next":"ok"},{"label":"先改 src/infrastructure 加功能","next":"src"}]},{"id":"bad","result":"不要。","detail":"门面见第三章 Nginx；证书见主机运维 TLS。"},{"id":"ok","result":"对。清单只是开演条件。","detail":"鸟瞰 → Core 放码 → HTTP/www。"},{"id":"src","result":"业务进 core/，勿先改 Runtime。","detail":"缺框架能力再提扩 src。"}]}
\`\`\`
