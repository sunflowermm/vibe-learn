...
| 能力 | 成功信号 |
|------|----------|
| 分层 | 分清终端仿真器 vs Shell 方言 vs OS |
| 本机 | 能说出当前用的是 Git Bash / PowerShell / … |
| PATH | 知道命令「找不着」常先查 PATH |
| 跟 Agent | 贴 \`node -v\` 前先说明用的哪一种壳 |
...
> 导图2 终端词条对齐口语；本课钉仿真器与方言不要混称。

| 导图2 | Vibe 口语 | 本课专业落点 |
|-------|-----------|--------------|
| **终端命令行** | 敲命令的窗口 | 仿真器≠Shell；Git Bash≠PowerShell |
| **环境变量** | PATH 等 | 决定命令能否被找到 |
| **JavaScript** | 稍后跑 Node | 先把壳与 PATH 弄清再谈语言 |
